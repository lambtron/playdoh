...
plugins.push(function(options){
  return function(files, metalsmith){
    var file = resolve(process.cwd(), options.config);
    var conf = read(file);
...
plugins.push(function(options){
  return function*(files, metalsmith){
    ...
    var builds = yield metalsmith.read(resolve(__dirname, '../build'));
 ...
 * Sitemap template.
 *
 * HACK: this needs to come before all of the others so that the articles aren't
 * already templated when it's generated.
 *
 * @param {Object} options
 * @return {Function}
 */
